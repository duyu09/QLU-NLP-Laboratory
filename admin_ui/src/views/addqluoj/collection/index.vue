    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="所属类别" prop="configId">
        <el-select v-model="queryParams.configId" placeholder="请选择所属类别" clearable size="small">
          <el-option
            v-for="dict in dict.type.qlu_topic_collection"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['addqluoj:collection:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['addqluoj:collection:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['addqluoj:collection:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['addqluoj:collection:export']"
        >导出</el-button>
      </el-col>
...
<!--      <el-table-column label="主键" align="center" prop="id" />-->
      <el-table-column label="题集名称" align="center" prop="name" />
      <el-table-column label="题集内容" align="center" >
      ...
      <el-table-column label="所属类别" align="center" prop="configId">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.qlu_topic_collection" :value="scope.row.configId"/>
        </template>
      </el-table-column>
...
<!--        </el-form-item>-->
    ...
    <!--    详细内容预览框-->
    <el-dialog title="详细内容" :visible.sync="ifShowRecordContent" @close="closeRecordContent">
...
<script>
import { listCollection, getCollection, delCollection, addCollection, updateCollection } from "@/api/addqluoj/collection";

export default {
  name: "Collection",
  dicts: ['qlu_topic_collection', 'sys_normal_disable'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除题集资源管理编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('addqluoj/collection/export', {
        ...this.queryParams
