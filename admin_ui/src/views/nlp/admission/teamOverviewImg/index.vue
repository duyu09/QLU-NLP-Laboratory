<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="分类标题id" prop="titleId">
        <el-input
          v-model="queryParams.titleId"
          placeholder="请输入分类标题id"
          clearable
          size="small"
    ...
    <el-table v-loading="loading" :data="teamOverviewImgList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分类标题id" align="center" prop="titleId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="分类标题id" prop="titleId">
          <el-input v-model="form.titleId" placeholder="请输入分类标题id" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        titleId: [
          { required: true, message: "分类标题id不能为空", trigger: "blur" }
        ],
        postSort: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除团队掠影 图片 (img)编号为"' + ids + '"的数据项？').then(function() {
