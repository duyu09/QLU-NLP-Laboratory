...
<!--    <el-table v-loading="loading" :data="frontendRecourceList" @selection-change="handleSelectionChange">-->
...
<script>
import { listFrontendRecource, getFrontendRecource, delFrontendRecource, addFrontendRecource, updateFrontendRecource,getByType } from "@/api/nlp/frontend/recource";

export default {
  name: "FrontendRecource",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFrontendRecource(this.queryParams).then(response => {
        this.frontendRecourceList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFrontendRecource(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改学术资源";
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFrontendRecource(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFrontendRecource(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除学术资源编号为"' + ids + '"的数据项？').then(function() {
        return delFrontendRecource(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('smcCloud/frontendRecource/export', {
        ...this.queryParams
      }, `frontendRecource_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
